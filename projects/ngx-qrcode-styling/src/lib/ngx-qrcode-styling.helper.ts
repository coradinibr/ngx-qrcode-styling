...
export function drawQrcode(config: Options, container: HTMLElement | HTMLVideoElement | HTMLCanvasElement | SVGElement | any): AsyncSubject<any> {
    ...
    const ADD_FRAME_SVG_TO_ELEMENT = () => {
        const http = fetch(`https://raw.githubusercontent.com/id1945/ngx-qrcode-styling/main/svg/${config?.frameOptions?.style ?? 'style20'}.svg`, { method: 'GET' })
    ...
    const UPDATE_SIZE_SVG = () => {
        const s1 = element.querySelector(`#${config?.frameOptions?.style ?? 'style20'}-svg`);
